...
class DatabaseMigrator:
    ...
    def migrate(
        ...
        # Tables to migrate (matching actual SQLite database)
        tables = [
            'stations', 'trains_master', 'trains', 'train_routes',
            'train_schedule', 'train_fares', 'train_running_days',
            'trains_active', 'tenants', 'api_keys', 'routes',
            'jobs', 'audit_logs', 'schedules', 'fares', 'system_metrics'
        ]
...
class DatabaseMigrator:
    ...
    @staticmethod
    def migrate(postgres_url: str, sqlite_db: str, dry_run: bool = False, backup_first: bool = True):
        """Migrate data from SQLite to PostgreSQL."""
    ...
    @staticmethod
    def verify(postgres_url: str, sqlite_db: str):
        """Verify migration consistency."""
        print(f"Verifying migration from {sqlite_db} to {postgres_url}")
