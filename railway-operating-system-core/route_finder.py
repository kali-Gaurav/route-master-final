    ...
    def find_one_transfer_routes(self, source, destination, start_date=None, max_results=100, verbose=False):
        ...
        with DatabaseConnection(self.db_path) as db:
            ...
            for junction in junctions:
                ...
                # Combine routes (limit combinations to avoid explosion)
                for train1 in leg1_trains[:10]:
                    for train2 in leg2_trains[:10]:
                        # Calculate transfer waiting time with day awareness
                        ...
                        # compute day offsets: leg1 departure day = 0 (start), leg1 arrival day = leg1['day_diff']
                        leg1_arrival_day = train1.get('day_diff', 0)
                        dep2_day_offset = leg1_arrival_day + (wait_day_offset or 0)

                        # If waiting invalid (too short), record skip with details
                        if not wait_valid:
                            if verbose:
                                base = start_date or datetime.today().date()
                                try:
                                    target_date = (base + timedelta(days=dep2_day_offset)).isoformat()
                                except:
                                    target_date = str(base)
                                skips.append({
                                    'type': 'one_transfer',
                        ...
                        # Validate that connecting train actually runs on that day
                        if not self.train_runs_on(train2['train_no'], base_date=start_date, day_offset=dep2_day_offset):
                            # skip this connection - train does not run on the computed transfer day
                            if verbose:
                                base = start_date or datetime.today().date()
                                try:
                                    target_date = (base + timedelta(days=dep2_day_offset)).isoformat()
                                ...
                                skips.append({
                                    ...
                                    'train2_departure': train2.get('departure'),
                                    'target_date': target_date,
                                    'reason': f"Train {train2['train_no']} does not run on {target_date}"
                                })
                            continue

                        # Calculate total travel time (on-board + waiting)
            ...
            for j1 in first_junctions[:15]:
                ...
                for j2 in second_junctions[:10]:
                    ...
                    for t1 in leg1[:5]:
                        for t2 in leg2[:5]:
                            for t3 in leg3[:5]:
                                # Calculate waiting times with day awareness (returns validity flag)
                                ...
                                # leg1 arrival day offset (relative to start_date)
                                leg1_arrival_day = t1.get('day_diff', 0)
                                # t2 departs on start_date + leg1_arrival_day + wait1_day_offset
                                ...
                                if not wait1_valid:
                                    if verbose:
                                        base = start_date or datetime.today().date()
                                        try:
                                            target_date = (base + timedelta(days=t2_dep_day_offset)).isoformat()
                                        except:
                                            target_date = str(base)
                                        skips.append({
                                            'type': 'two_transfer',
                                ...
                                # Validate t2 runs on computed day
                                if not self.train_runs_on(t2['train_no'], base_date=start_date, day_offset=t2_dep_day_offset):
                                    if verbose:
                                        base = start_date or datetime.today().date()
                                        try:
                                            target_date = (base + timedelta(days=t2_dep_day_offset)).isoformat()
                                        ...
                                        skips.append({
                                            ...
                                            'failed_departure': t2.get('departure'),
                                            'target_date': target_date,
                                            'reason': f"Train {t2['train_no']} does not run on {target_date}"
                                        })
                                    continue
                                ...
                                # t2 arrival day offset = t2_dep_day_offset + t2['day_diff']
                                t2_arrival_day = t2_dep_day_offset + t2.get('day_diff', 0)
                                # t3 departs on start_date + t2_arrival_day + wait2_day_offset
                                ...
                                if not wait2_valid:
                                    if verbose:
                                        base = start_date or datetime.today().date()
                                        try:
                                            target_date = (base + timedelta(days=t3_dep_day_offset)).isoformat()
                                        except:
                                            target_date = str(base)
                                        skips.append({
                                            'type': 'two_transfer',
                                ...
                                # Validate t3 runs on computed day
                                if not self.train_runs_on(t3['train_no'], base_date=start_date, day_offset=t3_dep_day_offset):
                                    if verbose:
                                        base = start_date or datetime.today().date()
                                        try:
                                            target_date = (base + timedelta(days=t3_dep_day_offset)).isoformat()
                                        ...
                                        skips.append({
                                            ...
                                            'failed_departure': t3.get('departure'),
                                            'target_date': target_date,
                                            'reason': f"Train {t3['train_no']} does not run on {target_date}"
                                        })
                                    continue

                                # For display, compute human-friendly day (1-based)
                                t2_day_display = 1 + t2_dep_day_offset
                                t3_day_display = 1 + t3_dep_day_offset
                                
                                # Calculate totals
                                total_time_minutes = t1['time_minutes'] + t2['time_minutes'] + t3['time_minutes'] + wait1_min + wait2_min
                                ...
                                routes.append({
                                    ...
                                    'total_distance': total_distance,
                                    'waiting_times': [wait1_str, wait2_str],
                                    'total_waiting_minutes': wait1_min + wait2_min,
                                    'total_time_minutes': total_time_minutes,
                                    'total_time_str': total_time_str
                                })
                    ...
                    for j3 in third_junctions[:5]:
                        ...
                        for t1 in leg1[:3]:
                            for t2 in leg2[:3]:
                                for t3 in leg3[:3]:
                                    for t4 in leg4[:3]:
                                        # Calculate waiting times with day awareness (valid flag included)
                                        wait1_result = self.calculate_transfer_waiting(t1['arrival'], t2['departure'])
                                        ...
                                        if not wait3_valid:
                                            if verbose:
                                                base = start_date or datetime.today().date()
                                                try:
                                                    target_date = (base + timedelta(days=t4_dep_day_offset if 't4_dep_day_offset' in locals() else 0)).isoformat()
                                                except:
                                                    target_date = str(base)
                                                skips.append({
                                                    'type': 'three_transfer',
                                        ...
                                        # t3 arrival day offset
                                        t3_arrival_day = t3_dep_day_offset + t3.get('day_diff', 0)
                                        # t4 departs on start_date + t3_arrival_day + wait3_day_offset
                                        t4_dep_day_offset = t3_arrival_day + (wait3_day_offset or 0)

                                        # Validate t4 runs on computed day
                                        if not self.train_runs_on(t4['train_no'], base_date=start_date, day_offset=t4_dep_day_offset):
                                            if verbose:
                                                base = start_date or datetime.today().date()
                                                try:
                                                    target_date = (base + timedelta(days=t4_dep_day_offset)).isoformat()
                                                except:
                                                    target_date = str(base)
                                                skips.append({
                                                    'type': 'three_transfer',
                                                    ...
                                                    'failed_departure': t4.get('departure'),
                                                    'target_date': target_date,
                                                    'reason': f"Train {t4['train_no']} does not run on {target_date}"
                                                })
                                            continue
                                            continue

                                        # For display, human-friendly day numbers
                                        t2_day = 1 + t2_dep_day_offset
                                        t3_day = 1 + t3_dep_day_offset
                                        t4_day = 1 + t4_dep_day_offset
                                        
                                        # Calculate totals
                                        total_time_minutes = t1['time_minutes'] + t2['time_minutes'] + t3['time_minutes'] + t4['time_minutes'] + wait1 + wait2 + wait3
                                        ...
                                        routes.append({
                                            ...
                                            'total_distance': total_distance,
                                            'waiting_times': [wait1_str, wait2_str, wait3_str],
                                            'total_waiting_minutes': wait1 + wait2 + wait3,
                                            'total_time_minutes': total_time_minutes,
                                            'total_time_str': total_time_str
                                        })
