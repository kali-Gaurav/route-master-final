...
import { ArrowLeftRight, CalendarDays, Search, Sparkles, Users, MapPin, Filter, Shuffle } from "lucide-react";
...
const Index = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const searchDate = travelDate;
      const dayOfWeek = new Date(searchDate).toLocaleDateString('en-US', { weekday: 'long' });

      console.log(`[Search] Searching for ${dayOfWeek} (${searchDate})`);

      let url = getApiUrl(
        `/api/routes?origin=${origin.code}&destination=${destination.code}&date=${searchDate}&max_transfers=${maxTransfers}`
      );
      ...
      const responseTime = endTime - startTime;
        ...
        // Map routes
        const mappedOptimal = (data.optimal_routes || [])
        ...
        const allResults = [...mappedOptimal, ...mappedAlternatives];

        const totalTime = endTime - startTime;

        // Top 10 optimal (fastest routes)
        const optimal = allResults.slice(0, 10);
        const alternatives = allResults.slice(10);

        setOptimalRoutes(optimal);
        setAllRoutes(allResults);
        setViewMode("optimal");
        setDisplayedAlternatives(5);

        console.log(
          `[Search] Completed in ${totalTime.toFixed(0)}ms. ` +
          `Found ${optimal.length} optimal + ${alternatives.length} alternatives`
        );

        toast({
          title: "Routes Found!",
          description: `Found ${optimal.length} optimal routes and ${alternatives.length} alternatives.`,
        } as Toast);
            ...
            <div className="bg-card rounded-3xl border-2 border-border p-6 md:p-8 shadow-card">
              <div className="relative mb-6">
                ...
                <button
                  ...
                  className={cn(
                    "absolute top-10 left-1/2 -translate-x-1/2 z-20 hidden md:flex",
                    "w-10 h-10 rounded-full bg-primary border-2 border-primary",
                    "items-center justify-center shadow-soft",
                    "hover:bg-primary/90 hover:border-primary transition-all"
                  )}
                >
              ...
              <div className="grid md:grid-cols-3 gap-4 mb-6">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select
                      className={cn(
                        "w-full pl-10 pr-4 py-2 rounded-xl appearance-none",
                        "bg-secondary border-2 border-transparent",
                        "text-foreground",
                        "focus:border-primary focus:ring-4 focus:ring-primary/10",
                        "transition-all duration-200 outline-none",
                ...
                <div className="flex items-end">
                  <button
                    ...
                    className={cn(
                      "w-full py-2 px-6 rounded-xl font-semibold text-base",
              ...
              <div className="mb-6 pb-6 border-b border-border">
                <label className="block text-sm font-medium text-muted-foreground mb-3">
                  Maximum Transfers: <span className="text-primary font-bold">{maxTransfers}</span>
                </label>
                <input
                  type="range"
                  min="0"
